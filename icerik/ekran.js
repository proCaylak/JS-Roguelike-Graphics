...
$ekran.startScreen = {
    ...
    render: function(display){
        ...
        display.drawText(1,1, "Web Tabanlı Roguelike Oyun");
...
$ekran.playScreen = {    
    ...
    _gameEnded: false,
    _subScreen: null,
    _tip:"%c{blue}%b{white}E "+ "%c{white}%b{black}Eşya al"+
        "%c{blue}%b{white} D "+ "%c{white}%b{black}Mnz. Sald."+
        "%c{blue}%b{white} C "+ "%c{white}%b{black}İncele"+
        "%c{blue}%b{white} F "+ "%c{white}%b{black}Envntr"+
        "%c{blue}%b{white} V "+ "%c{white}%b{black}Yemek"+
        "%c{blue}%b{white} G "+ "%c{white}%b{black}Silah kuşan"+
        "%c{blue}%b{white} B "+ "%c{white}%b{black}Bak"+
        "%c{blue}%b{white} K "+ "%c{white}%b{black}YARDIM",
    enter: function(){        
        var width = 100;
        var height = 48;
        var depth = 6;
    ...
    render: function(display){
        ...
        display.drawText(0, screenHeight + 1, this._tip);
    },
        ...
        for(var x = topLeftX; x < topLeftX + screenWidth; x++){
            for(var y = topLeftY; y < topLeftY + screenHeight; y++){
                if(map.isExplored(x, y, currentDepth)){
                    ...
                    var chars = [].concat(glyph.getChar());
                    var foregrounds = [].concat(glyph.getForeground());
                    var backgrounds = [].concat(glyph.getBackground());

                    if(visibleCells[x + ',' + y]){
                        if(map.getTile(x, y, currentDepth) === Game.Tile.holeToCavernTile){
                            Game.sendMessage(this._player, "Gulyabani\'nin ini keşfedildi!");
                        }

                        var items = map.getItemsAt(x, y, currentDepth);
                        if(map.getEntityAt(x, y, currentDepth)){
                            glyph = map.getEntityAt(x, y, currentDepth);
                            chars.push(glyph.getChar());
                            foregrounds.push(glyph.getForeground());
                            backgrounds.push(glyph.getBackground());
                        }
                        else if(items){
    ...
    handleInput: function(inputType, inputData){
        ...
        if(inputType === 'keydown'){
            ...
            if(inputData.keyCode === ROT.VK_SHIFT){
                this._tip = "%c{blue}%b{white} D "+ "%c{white}%b{black}Mnz. Silah kuşan"+
                "%c{blue}%b{white} F "+ "%c{white}%b{black}Eşya bırak"+
                "%c{blue}%b{white} G "+ "%c{white}%b{black}Zırh kuşan";
                Game.refresh();
            }
            ...
            if(inputData.keyCode === ROT.VK_LEFT){
                ...
                if(inputData.shiftKey){
                    this.showItemsSubScreen($ekran.wieldThrowableScreen, this._player.getItems(),
                        'Envanterde fırlatılabilir eşya yok.');
                    return;
                }
                else{
                    if(this._player.getRangedAttackValue() > 0){
                        var offsets = this.getScreenOffsets();
                        $ekran.rangedAttackScreen.setup(this._player,
                            this._player.getX(), this._player.getY(),
                            offsets.x, offsets.y);
                        this.setSubScreen($ekran.rangedAttackScreen);
                    return;
                    }
                    else{
                        Game.sendMessage(this._player, "Elinizde fırlatılabilir eşya yok.");
                    }
                }
            }
            else{
            ...
            if(inputData.keyCode === ROT.VK_SHIFT){
                this._tip = "%c{blue}%b{white}E "+ "%c{white}%b{black}Eşya al"+
                "%c{blue}%b{white} D "+ "%c{white}%b{black}Mnz. Sald."+
                "%c{blue}%b{white} C "+ "%c{white}%b{black}İncele"+
                "%c{blue}%b{white} F "+ "%c{white}%b{black}Envntr"+
                "%c{blue}%b{white} V "+ "%c{white}%b{black}Yemek"+
...
$ekran.ItemListScreen.prototype.render = function(display){
    ...
    for(var i = 0; i < this._items.length; i++){
        if(this._items[i]){
            ...
            }else if(this._items[i] === this._player.getThrowable()){
                suffix = ' (mevcut kuşanılan menzilli)';
            }
...
$ekran.wieldThrowableScreen = new $ekran.ItemListScreen({
    caption: 'Kuşanmak istediğin menzilli fırlatılabilir eşyayı seç:',
    canSelect: true,
    canSelectMultipleItems: false,
    hasNoItemOption: true,
    isAcceptable: function(item){
        return item && item.hasMixin('Equippable') && item.isThrowable();
    },
    ok: function(selectedItems){
        var keys = Object.keys(selectedItems);
        if(keys.length === 0){
            this._player.holster();
            Game.sendMessage(this._player, 'Silahsızsın.');
        }else{
            var item = selectedItems[keys[0]];
            this._player.unequip(item);
            this._player.wieldThrowable(item);
            Game.sendMessage(this._player, "Eline %s aldın.", [item.describe()]);
        }
        return true;
    }
});
...
$hedef_ekran.render = function(display){
    ...
    for(var i = 0, l = points.length; i < l; i++){
        ...
        if(this._player.getMap().isExplored(x, y, z)){
            if(this._visibleCells[x + ',' + y]){
                var glyph = this._player.getMap().getTile(x, y, z);
                var chars = [].concat(glyph.getChar())
                var foregrounds = [].concat(glyph.getForeground());
                var backgrounds = [].concat(glyph.getBackground());
                chars.push('*');
                foregrounds.push('rgba(0, 160, 0, 1.0)');
                backgrounds.push('rgba(0, 0, 0, 0.0)');
                display.draw(points[i].x, points[i].y, chars, foregrounds, backgrounds);
            }
            else{
                var glyph = this._player.getMap().getTile(x, y, z);
                var chars = [].concat(glyph.getChar())
                var foregrounds = [].concat('rgba(45, 45, 45, 0.5)');
                var backgrounds = [].concat(glyph.getBackground());
                chars.push('*');
                foregrounds.push('rgba(160, 160, 0, 1.0)');
                backgrounds.push('rgba(0, 0, 0, 0.0)');
                display.draw(points[i].x, points[i].y, chars, foregrounds, backgrounds);
            }
        }
        else{
            display.draw(points[i].x, points[i].y, '*', 'rgba(160, 0, 0, 1.0)', 'black');
        }
    }
...
$hedef_ekran.executeOkFunction = function(){
    ...
    if (this._okFunction(this._cursorX + this._offsetX, this._cursorY + this._offsetY)) {
        var message = this._player.getMessages();
        this._player.getMap().getEngine().unlock();
        if (message){Game.sendMessage(this._player, message[0]);}
    }
};
...
$ekran.rangedAttackScreen = new $ekran.TargetBasedScreen({
    captionFunction: function(x, y){
        var z = this._player.getZ();
        var map = this._player.getMap();

        if(this._visibleCells[x + ',' + y]){
            if(map.getEntityAt(x, y, z) && map.getEntityAt(x,y,z) != this._player){
                var entity = map.getEntityAt(x, y, z);
                return String.format('Hedef: %s - %s (%s)',
                    entity.getRepresentation(),
                    entity.describe(),
                    entity.details());
            }
            else{
                return "Hedeflenen yerde düşman varlık bulunamadı.";
            }
        }
        return "Görüş alanının dışında.";
    },
    okFunction: function(x, y){
        ...
        if(this._visibleCells[x + ',' + y]){
            ...
            if(target && target != this._player){
                this._player.rangedAttack(target);
                if(this._player.getThrowable() != null){
                    var ammo = this._player.getThrowable().getAmmoValue();
                    this._player.getThrowable().setAmmoValue(--ammo);
                    if(ammo <= 0){
                        for(var key in this._player.getItems()){
                            if(this._player.getItem(key) === this._player.getThrowable()){
                                this._player.removeItem(key);
                                break;
                            }
                        }
...
$ekran.helpScreen = {
    ...
    render: function(display){
        ...
        display.drawText(0, y++, 'Bir çıkış yolunu bulmaya çalışırken ayakların kaydı');
        display.drawText(0, y++, 've derin bir çukurdan düştün.');
        ...
        y += 2;
        display.drawText(0, y++, '[ok tuşları] Hareket et/ Saldır');
        ...
        display.drawText(0, y++, '[D] Görüş mesafesi içinde menzilli saldırı yap(Menzilli eşya gerektirir)');
        display.drawText(0, y++, '[SHIFT]+[D] Menzilli fırlatılabilir eşya kuşan.');
