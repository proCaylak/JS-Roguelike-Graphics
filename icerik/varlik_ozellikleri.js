...
$varlik_ozellik.InventoryHolder = {
    ...
    init: function(template){
        var inventorySlots = template['inventorySlots'] || 3;
...
$varlik_ozellik.TaskActor = {
    ...
    canDoTask: function(task){//DEVAM EDECEK
        if(task === 'hunt'){
    ...
    hunt: function(){//DEVAM EDECEK
        ...
        }else if(this.hasMixin('Attacker') && this.getRangedAttackValue() > 0){
            this.rangedAttack(player);
            if(this.getThrowable() != null){
                var ammo = this.getThrowable().getAmmoValue();
                this.getThrowable().setAmmoValue(--ammo);
                if(ammo <= 0){
                    for(var key in this.getItems()){
                        if(this.getItem(key) === this.getThrowable()){
                            this.removeItem(key);
                            break;
                        }
                    }
...
$varlik_ozellik.Attacker = {
    ...
    getRangedAttackValue: function(){
        ...
        if(this.hasMixin($varlik_ozellik.Equipper)){
            if(this.getThrowable()){
                modifier += this.getThrowable().getRangedAttackValue();
            }
        }
    ...
    attack: function(target){
        if(target.hasMixin('Destructible')){
            ...
            Game.sendMessage(target, '%s tarafından %d hasar aldın! (yakın dövüş)',
                [this.getName(), damage]);
    ...
    rangedAttack: function(target){
        if(target.hasMixin('Destructible')){
            var ranged = this.getRangedAttackValue();
            var defense = Math.round((target.getDefenseValue() * 0.60));
            var max = Math.max(0, ranged - defense);
            var damage = 1 + Math.floor(Math.random() * max);
            if(this.getThrowable() != null){
                Game.sendMessage(this, '%s atarak %s hedefine %d hasar verdin!',
                    [this.getThrowable().getName(), target.getName(), damage]);
            }else{
                Game.sendMessage(this, 'Menzilli saldırı ile %s hedefine %d hasar verdin!',
                    [target.getName(), damage]);
            }
            Game.sendMessage(target, '%s tarafından %d hasar aldın! (menzilli)',
                [this.getName(), damage]);
...
$varlik_ozellik.CorpseDropper = {
    ...
    listeners:{
        onDeath: function(attacker){
            if(this.hasMixin($varlik_ozellik.InventoryHolder)){//DEVAM EDECEK
...
$varlik_ozellik.Equipper = {
    ...
    wieldThrowable: function(item){
        this._throwable = item;
    },
    holster: function(){
        this._throwable = null;
    },
...
$varlik_ozellik.ExperienceGainer = {
    ...
    init: function(template){
        ...
        if(this.hasMixin('Sight')){
            this._statOptions.push(['Görüş ve saldırı menzilini artır.', this.increaseSightRadius]);
        }
    },
    getLevel: function(){
        return this._level;
